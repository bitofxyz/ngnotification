/**
 * Created by guoyinfeng on 11/12/14.
 */
angular.module('notifications', []).factory('$notification', ['$timeout', function ($timeout) {
    'use strict';
    ...
    var notifications = JSON.parse(localStorage.getItem('$notifications')) || [],
        queue = [];
    var settings = {
        type_info: {duration: 5000, enabled: true},
        type_warning: {duration: 5000, enabled: true},
        type_error: {duration: 5000, enabled: true},
        type_success: {duration: 5000, enabled: true},
...
// Returns all notifications that are currently stored
        ...
        info: function (title, content, userData) {
            ...
            return this.awesomeNotify('type_info', 'info', title, content, userData);
        },
        error: function (title, content, userData) {
            return this.awesomeNotify('type_error', 'remove', title, content, userData);
        },
        success: function (title, content, userData) {
            return this.awesomeNotify('type_success', 'check', title, content, userData);
        },
        warning: function (title, content, userData) {
            return this.awesomeNotify('type_warning', 'exclamation', title, content, userData);
        },
...
// Load all notifications from localStorage
    ...
    directive('notifications', ['$notification', '$compile', function ($notification, $compile) {
        ...
        var html =
            ...
            '<p>{{noti.content}}</p>' +
            '<div class="bigboxicon"><i class="fa fa-{{noti.icon}} swing animated"></i></div>' +
            '</div>' +
            ' </div>';
